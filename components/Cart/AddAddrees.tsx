  ...
  return (
    <div className={`fixed w-full left-0 z-50 h-full flex justify-center top-0 py-20 ${props.AddreesPopup?"":"hidden"}`}>
        <form onSubmit={(e) => HandleSubmit(e)} className='w-9/12 text-white flex flex-col gap-6 bg-black/85 border p-14 rounded-lg h-full relative'>
            ...
            <input onChange={(e) => SetFName(e.target.value)} required className='bg-transparent px-2 ouline-none border-b h-10' placeholder='Nama Depan' type="text" />
            <input onChange={(e) => SetLName(e.target.value)} required className='bg-transparent px-2 ouline-none border-b h-10' placeholder='Nama Belakang' type="text" />
            <input onChange={(e) => SetPhone(e.target.value)} required className='bg-transparent px-2 ouline-none border-b h-10' placeholder='Phone' type="text" />
            <input onChange={(e) => SetEmail(e.target.value)} className='bg-transparent px-2 ouline-none border-b h-10' placeholder='Email' type="email" />
            <Addrees/>
